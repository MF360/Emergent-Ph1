...
import TaskColumn, { type Column } from "./TaskColumn";
...
const TaskBoard: React.FC<TaskBoardProps> = ({ className = "" }) => {
  ...
  const handleTaskDragStart = (e: React.DragEvent, task: Task) => {
    ...
    // Store source column in drag event
    const sourceColumn = columns.find((col) =>
      col.tasks.some((t) => t.id === task.id)
    );
    if (sourceColumn) e.dataTransfer.setData("sourceColumnId", sourceColumn.id);
  };

  const handleTaskDragEnd = () => setDraggedTask(null);

  const handleDragOver = (e: React.DragEvent) => e.preventDefault();
  const handleDragLeave = (_e: React.DragEvent) => {}; // unused param
